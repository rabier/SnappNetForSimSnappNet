// it is the file Data.java taken from snapp
/*
 * File Data.java
 *
...
public class SnapData extends beast.evolution.alignment.Alignment {
		...
		if (m_taxonsets.get().size() > 0) {
			...
			if (rawDataType instanceof Binary || rawDataType instanceof IntegerData || rawDataType instanceof StandardData) {
				...
				for(TaxonSet set : m_taxonsets.get()) {
					//System.out.println("Ds SnapData, voici le taxon set" +  set + "\n");
					...
					for (Taxon taxon : set.taxonsetInput.get()) {
						//System.out.println("et voici son taxon " +  taxon + "\n");
						...
						for (int i = 0; i < sequences.size() && !bFound; i++) {
							if (sequences.get(i).taxonInput.get().equals(taxon.getID())) {
								...
								bFound = true;
								//CE
								//System.out.println("Voici la sequence associee" +  SNPSequence.m_sequences.get() + "\n");
								 //END CE
							}
						}
						if (!bFound) {
					...
					//CE
					//System.out.println("Voici le nbre totalcount" +  SNPSequence.totalCountInput.get()+ "\n");
					//END CE
