...
//            new Input<>("checkpoint", "keep saved states (every X samples).", false);
     ...
     */
    public void restore() {
    	Log.debug.println("J'arrive ds RESTORE"); 
    	Log.debug.println("Voila le nbre de Changed StateNodes " +  nrOfChangedStateNodes); 
    	
        for (int i = 0; i < nrOfChangedStateNodes; i++) {
        	//Log.debug.println("i est egale a " +  i + "\n");
        	Log.debug.println("changeStateNodes["+i+"] est egal a " +  changeStateNodes[i]);
        	
        	Log.debug.println("Je restore le stateNode d id" + i + "qui est" + stateNode[changeStateNodes[i]]); 
            stateNode[changeStateNodes[i]].restore();
        }
        Log.debug.println("Je quitte RESTORE\n"); 
    }
    ...
    public void storeCalculationNodes() {
        ...
        for (final CalculationNode calculationNode : currentSetOfCalculationNodes) {
        	...
        	Log.debug.println("Je suis le calculationNode intitule " +  calculationNode);     
