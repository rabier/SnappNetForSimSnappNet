...
public class MCMC extends Runnable {
    ...
    public void initAndValidate() {
        ...
        //CE
        Log.debug.printf("Je suis au debut de initAndValidate du MCMC.java de Beast2\n");
        //END CE
        ...
        //CE
        //Log.debug.printf("J ai fini de recuperer tous mes operateurs ds initAndValidate du MCMC.java de Beast2\n");
        //END CE
    ...
    public void run() throws IOException, SAXException, ParserConfigurationException, InterruptedException {
    	...
    	//CE
    	/// warning j ai ajoute InterruptedException au dessus pour pouvoir mettre des sleep
        Log.debug.printf("Je suis au debut du run de MCMC.java de Beast\n");
        //Thread.sleep(20000);
        //END CE	
    ...
    protected void doLoop() throws IOException {
        ...
        for (int sampleNr = -burnIn; sampleNr <= chainLength; sampleNr++) {
            ...
            //if (debugFlag && sampleNr % 3 == 0 || sampleNr % 1  == 0) {
                ...
                if (sampleNr >= 0) {
                	Log.debug.println("Je fais mon optimisation de l operateur ds do Loop de MCMC");
            ...
            if (posterior.getCurrentLogP() == Double.POSITIVE_INFINITY) {
     		...
     		// Instructions pour calculer le temps mis par cette operation
    		currentTime = System.currentTimeMillis();
    		double diff =  ((double)(currentTime-previousTime))/1000.0;
    		previousTime = currentTime;
    		Log.debug.println("temps pour l'iteration suite a l'operateur indique ci-dessus) = "+ diff + "\n");      
...
//            }
	...
	//CE
	Log.debug.printf("Voici le nom de l operateur choisi ds MCMC.java de beast2"+ operator.getName() + "\n");
	Log.debug.printf("Voici mon weight associe"+ operator.getWeight() + "\n");
	// ENDCE
        ...
        if (evaluatorDistribution != null) {
            evaluator = new Evaluator() {
                ...
                public double evaluate() {
                    ...
                    try {
                    	Log.debug.println("MCMC.evaluate() lance calcul likelihood");
                        logP = evaluatorDistribution.calculateLogP();
                        Log.debug.printf("Je passe la ds evaluatorDistribution !!!\n");
                    } catch (Exception e) {
                        e.printStackTrace();
                        System.exit(1);
                    }                  
        ...
        if (logHastingsRatio != Double.NEGATIVE_INFINITY) {
            if (operator.requiresStateInitialisation()) {
            	Log.debug.println("Je passe dans requiresStataInitialisation");  
                state.storeCalculationNodes();
                state.checkCalculationNodesDirtiness();
            }

            Log.debug.println("propagateState lance calcul likelihood");	
            ...
            if (logAlpha >= 0 || Randomizer.nextDouble() < Math.exp(logAlpha)) {
            	...
            	Log.debug.println("J ai accepte !!!!\n"); 
                //Log.info.println("J ai accepte !!!!\n");
                ...
                // reject
                Log.debug.println("J ai rejete !!!!\n");  
                //Log.info.println("J ai rejete !!!!\n");
                if (sampleNr >= 0) {
                    operator.reject(newLogLikelihood == Double.NEGATIVE_INFINITY ? -1 : 0);
                }
                //CE
                Log.debug.println("Il existe "+ state.getNrOfStateNodes() +" stateNodes\n");  
                for (int i=0; i<state.getNrOfStateNodes(); i++) {
                	Log.debug.println("Voici le " + i + "eme state node --> "+ state.getStateNode(i));                 	
                }	               	
                //END CE
                ...
                //CE
                Log.debug.println("Regardons si je suis bien revenu a l etat ancien");  
                Log.debug.println("Il existe "+ state.getNrOfStateNodes() +" stateNodes");  
                for (int i=0; i<state.getNrOfStateNodes(); i++) {
                	Log.debug.println("Voici le " + i + "eme state node !!!!"+ state.getStateNode(i));                 	
                }	                  
            ...
            // operation failed
        	Log.debug.println("Hasting ratio Failed !");
            if (sampleNr >= 0) {
                operator.reject(-2);
            }
            state.restore();
    ...
    protected void reportLogLikelihoods(final Distribution distr, final String tabString) {
        ...
        Log.debug.println(tabString + "P(" + distr.getID() + ") = " + full + " (was " + last + ")" + changed);
