...
//        }
    ...
    public double calculateLogP() {
    	Log.debug.println("calculateLogP de CompoundDistrib");
        logP = 0;
        if (ignore) {
        	return logP;
        }
        ...
        if (useThreads && workAvailable > 1) {
            ...
            for (Distribution dists : pDistributions.get()) {
            	Log.debug.println("Je suis la distribution" + dists);
                if (dists.isDirtyCalculation()) {                	
                	Log.debug.println("Je suis dirty !");
                    logP += dists.calculateLogP();
                } else {
                	Log.debug.println("Je ne suis pas dirty !");
