...
//            infoLogger.info("  File encoding: " + fileReader.getEncoding());
            ...
            logger.addHandler(handler);
            System.out.printf("Je suis dans beastMain, juste avant de lancer beastMCMC.run() \n");
            beastMCMC.run();
...
//                        new Arguments.Option("strict", "Fail on non-conforming BEAST XML file"),
	    ...
	    System.out.println("Je suis juste dans le parser");//CE
	...
	System.out.println("Je suis sorti du try");//CE
...
//        }
	...
	System.out.println("Je suis juste avant la creation de BeastMCMC (in BeastMain.java)");//CE
        // Construct the beast object
        final BeastMCMC beastMCMC = new BeastMCMC();
	System.out.println("Je suis juste apres la creation de BeastMCMC (in BeastMain.java)");//CE

        try {
            // set all the settings...
            MCMCargs.add(inputFile.getAbsolutePath());
            beastMCMC.parseArgs(MCMCargs.toArray(new String[0]));
	    System.out.println("Je suis dans BeastMain.java (j ai fini de parser mon xml)");//CE

            if (!doNotRun) {
		System.out.println("Je passe la, je vais bien lancer le run!!"); //CE
