...
public class Sequence extends BEASTObject {
    ...
    public List<Integer> getSequence(DataType dataType) {
    	...
    	if (uncertain) {
	        ...
	        // remove spaces
	        data = data.replaceAll("\\s", "");	        
	        Log.debug.println("Je suis juste apres replaceAll ds Sequence.java \n");
	        sequence = dataType.string2state(data);
	        Log.debug.print("la sequence transformee en entiers est : ");
	        Log.debug.println(sequence);
    	}
        ...
        Log.debug.print("et elle a un totalcount = ");
        Log.debug.println(totalCountInput.get());
        return sequence;
    }
