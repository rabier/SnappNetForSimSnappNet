...
public class Alignment extends Map<String> {
     ...
     * If site weights and/or data type have been previously set up with initAndValidate then they
    ...
    protected void calcPatterns() {
         ...
         * VB+CE : ici un SNP '11?0' est un pattern different du SNP '1100' meme si les deux ont le meme nb de rouges et verts
         * *
         */
    private void calcPatterns(boolean log) {
        ...
        // sort data : hyp VB met cote a cote les sites identiques 
        SiteComparator comparator = new SiteComparator();
        Arrays.sort(data, comparator);

        // count patterns in sorted data (VB : et compresse la matrice pour ne garder qu'une seule fois chq ligne (ie chq site different)
        ...
        if (stripInvariantSitesInput.get()) {
            // don't add patterns that are invariant, e.g. all gaps (tout 0s ou tout 1s)
